...
C4's complexity resides somewhere between vanilla Emacs and a full-fledged distribution like
Spacemacs or configuration framework like Doom Emacs.
...
C4 requires straight.el and use-package, so that gets loaded in first.
...
After package setup, we need to load in Org Mode to trigger our configuration as well as
rebuild it on save.
...
The =preamble.el= module does some startup optimizations and housekeeping to keep the
Emacs directory free of clutter.
...
Now we'll explore what the functions actually do.
...
To use the keybindings, it's necessary to load them in.
